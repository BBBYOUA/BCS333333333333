...
def get_crazy_functions():
    ...
    {
        "插件显示在WEB界面的名字": {
            "Color": "stop",                 # 插件的颜色（stop或secondary，前者红色，代表需要读取输入区; 后者白色，代表不读取输入区）
            "AsButton": False,               # True代表作为`按钮`加入WEB界面，False代表作为`下拉菜单的一员`加入WEB界面
            "AdvancedArgs": True,            # True代表在选中后需要弹出`高级参数输入区`（例如生成图像插件中，用于输入分辨率）
            "ArgsReminder": "",              # `高级参数输入区`（如启用）显示给用户的提示文本，例如"分辨率的输入格式是 123x123"
            "Function": HotReload(函数入口)   # 函数入口。HotReload 的意思是热更新，修改函数插件后，不需要重启程序，代码直接生效。如插件中有全局变量，慎用HotReload
        }
    }
    ...
    function_plugins = {
        ...
        "[测试功能] 解析Jupyter Notebook文件": {
            ...
            "AdvancedArgs": True,  
            "ArgsReminder": "若输入0，则不解析notebook中的Markdown块", 
        },
        "批量总结Word文档": {
            "Color": "stop",
    ...
    try:
        ...
        function_plugins.update({
            "图片生成（先切换模型到openai或api2d）": {
                ...
                "AsButton": False,
                "AdvancedArgs": True,
                "ArgsReminder": "在这里输入分辨率, 如256x256（默认）", 
